...
function loadStatsPage(){
	...
	for(var l = 2; l < x.rows.length; l++){
		var a = parseInt(x.rows[l].cells[2].innerHTML);
		var b = parseInt(x.rows[l].cells[3].innerHTML);
		
		if (a > b){
			x.rows[l].cells[4].innerHTML = "CU Boulder";
			sumval++;
		}
		else{
			diff++;
			x.rows[l].cells[4].innerHTML = x.rows[l].cells[1].innerHTML;
		}
	}
	
	var c = document.getElementById("wins");
	c.innerHTML = sumval;
	
	var d = document.getElementById("losses");
	d.innerHTML = diff;
}
...
function loadPlayersPage(){
	...
	for(var l = 0; l < players.length; l++){
		...
		player.href = "#";
		player.setAttribute("onclick", "switchPlayers("+l+")");
		player.innerHTML = players[l].name;
		console.log(player);
